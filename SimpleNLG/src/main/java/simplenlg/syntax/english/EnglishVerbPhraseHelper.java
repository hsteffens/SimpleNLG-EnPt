 ...
 * @version 4.0
 */
public class EnglishVerbPhraseHelper extends AbstractVerbPhraseHelper{
	...
	public final Stack<NLGElement> createVerbGroup(
		...
		if (Form.INFINITIVE.equals(formValue)) {
			...
			if (Tense.FUTURE.equals(tenseValue)
					&& modal == null
					&& ((!(phrase.getHead() instanceof AbstractCoordinatedPhraseElement)) || (phrase
							.getHead() instanceof AbstractCoordinatedPhraseElement && interrogative))) {
	...
	private void pushFrontVerb(PhraseElement phrase,
		...
		if (Form.GERUND.equals(formValue)) {
			...
			if (!(EnglishInterrogativeType.WHO_SUBJECT.equals(interrogType) || EnglishInterrogativeType.WHAT_SUBJECT
					.equals(interrogType))) {
			...
			if (!(phrase.getFeatureAsBoolean(Feature.NEGATED).booleanValue() && (EnglishInterrogativeType.WHO_OBJECT
					.equals(interrogType) || EnglishInterrogativeType.WHAT_OBJECT
					.equals(interrogType)))) {
				vgComponents.push(frontVG);
			}
		}
	...
	private NLGElement createNot(PhraseElement phrase,
		...
		if (phrase.getFeatureAsBoolean(Feature.NEGATED).booleanValue()) {
			...
			boolean addDo = !(EnglishInterrogativeType.WHAT_OBJECT.equals(interrType) || EnglishInterrogativeType.WHO_OBJECT
					.equals(interrType));
	 ...
	 */
	private NLGElement grabHeadVerb(PhraseElement phrase,
	 ...
	 *            the stack of verb components in the verb group.
	 */
	private void pushParticles(PhraseElement phrase,
		...
		if (parent instanceof PhraseElement) {
			if (parent.isA(PhraseCategory.CLAUSE)
					&& (PhraseHelper.isExpletiveSubject((PhraseElement) parent)
							|| EnglishInterrogativeType.WHO_SUBJECT.equals(parent
									.getFeature(Feature.INTERROGATIVE_TYPE)) || EnglishInterrogativeType.WHAT_SUBJECT
							.equals(parent
	...
	public void realiseComplements(SyntaxProcessor parent,
		...
		EnglishInterrogativeType interrogativeType = (EnglishInterrogativeType) phrase.getFeature(Feature.INTERROGATIVE_TYPE);
		if (!EnglishInterrogativeType.WHO_INDIRECT_OBJECT.equals(interrogativeType)) {
			realisedElement.addComponents(indirects.getChildren());
		}
		if (!phrase.getFeatureAsBoolean(Feature.PASSIVE).booleanValue()) {
			if (!(EnglishInterrogativeType.WHO_OBJECT.equals(interrogativeType) || EnglishInterrogativeType.WHAT_OBJECT.equals(interrogativeType))) {
