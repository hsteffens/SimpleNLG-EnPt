...
import simplenlg.syntax.english.EnglishClauseHelper;
import simplenlg.syntax.english.EnglishVerbPhraseHelper;
import simplenlg.syntax.english.EnglishNounPhraseHelper;
...
public class SyntaxProcessor extends NLGModule {
	...
	private AbstractNounPhraseHelper nounPhraseHelper;
	private AbstractClauseHelper clauseHelper;
	
	public SyntaxProcessor() {
		this.verbPhraseHelper = new EnglishVerbPhraseHelper();
		this.nounPhraseHelper = new EnglishNounPhraseHelper(); 	 
	}
	
	public SyntaxProcessor(AbstractVerbPhraseHelper verbPhraseHelper, AbstractNounPhraseHelper nounPhraseHelper, AbstractClauseHelper clauseHelper) {
		this.verbPhraseHelper = verbPhraseHelper;
		this.nounPhraseHelper = nounPhraseHelper;
		this.clauseHelper = clauseHelper;
	}

	@Override
	public void initialise() {
		if (clauseHelper == null) {
			clauseHelper = new EnglishClauseHelper(verbPhraseHelper);
		}
	}
		...
		if (element != null
			...
			if (element instanceof DocumentElement) {
				...
				realisedElement = CoordinatedPhraseHelper.realise(this,
						(AbstractCoordinatedPhraseElement) element);
