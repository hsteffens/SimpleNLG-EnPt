...
public abstract class AggregationRule {
	...
	public AggregationRule() {
		this.factory = new EnglishNLGFactory();
	}
	 ...
	 * {@link simplenlg.framework.AbstractCoordinatedPhraseElement}, in which case it
	...
	public NLGElement apply(NLGElement phrase) {
		...
		if (phrase instanceof AbstractCoordinatedPhraseElement) {			
			List<NLGElement> children = ((AbstractCoordinatedPhraseElement) phrase).getChildren();
			...
			if(aggregated.size() == 1) {
				...
				for (NLGElement agg : aggregated) {
					((AbstractCoordinatedPhraseElement) result).addCoordinate(agg);
				}				
			}			
		}
