...
import simplenlg.morphology.english.EnglishMorphologyProcessor;
import simplenlg.morphology.portuguese.PortugueseMorphologyProcessor;
...
import simplenlg.orthography.english.EnglishOrthographyProcessor;
import simplenlg.orthography.portugues.PortugueseOrthographyProcessor;
import simplenlg.syntax.SyntaxProcessor;
import simplenlg.syntax.portuguese.PortugueseClauseHelper;
import simplenlg.syntax.portuguese.PortugueseNounPhraseHelper;
import simplenlg.syntax.portuguese.PortugueseVerbPhraseHelper;
...
public class Realiser extends NLGModule {
	...
	private AbstractOrthographyProcessor orthography;
	...
	public Realiser(Lexicon lexicon) {
		if (lexicon.getLanguage() != null) {
			if (lexicon.getLanguage().equals(EnLanguage.PORTUGUESE)) {
				this.morphology = new PortugueseMorphologyProcessor(new PortugueseMorphologyRules());	
				this.syntax = new SyntaxProcessor(new PortugueseVerbPhraseHelper(), new PortugueseNounPhraseHelper(), new PortugueseClauseHelper(new PortugueseVerbPhraseHelper()));
				this.orthography = new PortugueseOrthographyProcessor();
			}else{
				this.morphology = new EnglishMorphologyProcessor();
				this.syntax = new SyntaxProcessor();
				this.orthography = new EnglishOrthographyProcessor();
			}
		}else{
			this.morphology = new EnglishMorphologyProcessor();
			this.syntax = new SyntaxProcessor();
	 ...
	 * {@link simplenlg.orthography.english.EnglishOrthographyProcessor} has the
	 * parameter set.
	 * 
	 ...
	 * parameter in the
	 * {@link simplenlg.orthography.english.EnglishOrthographyProcessor}.
	 * 
